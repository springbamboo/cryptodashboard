  ...
  for (let i = 0; i < coinNameHttp.length; i++) {
    ...
    request(http_endPoint + http_ratio, function(err, response, payload) {
      ...
      if (ratioData.data.list[1]) {
        pairs[pair.replace("-", "")].ratio.short = ratioData.data.list[1].sell_ratio;
        pairs[pair.replace("-", "")].ratio.long = ratioData.data.list[1].buy_ratio;
      }
    });
  }
}, 1000);
  ...
  if (jsonData.tick) {
    for(let i in pairs){
      ...
      if(i.toLowerCase() === coinName){
        ...
        pairs[i].change = parseFloat(jsonData.tick.close) / parseFloat(jsonData.tick.open) - 1;
      }
    }
  }
...
for (let i = 0; i < coinNameHttp.length; i++) {
  ...
  request(http_endPoint + http_funding, function(err, response, payload) {
    ...
    if (fundingData.data) {
      pairs[pair.replace("-", "")].funding = fundingData.data.funding_rate;
    }
    //console.log(pairs);
  });
}
